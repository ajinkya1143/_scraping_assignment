...
class Validation:
    ...
    def validate_pdp_data(self):
        for pdp_json in pdps_json:
            ...
            mandatory_fields = ['title', 'product_id', 'brand', 'original_price']
            mandatory_variant_fields = ['price', 'image']
    ...
    def validate_price(self,product_data):
        ...
        if selling_price is not None and original_price is not None:
            if not selling_price <= original_price:
                self.errors.append(f"Sale price {selling_price} is greater than original price {original_price} for product {product_data['product_id']}")

    def validate_mandatory_attributes(self,product_data,mandatory_fields):
        for key in mandatory_fields:
            if not product_data.get(key):
                self.errors.append(f"Missing mandatory field: {key} for product :{product_data['product_id']}")
...
if __name__ == "__main__":
    with open(r'C:\Users\ajinkya.ghodvinde\Desktop\adaptmind\scrape_data\output\foreignfortune_data.json',
              "r") as f:
